...
```{r 10_poly, echo=TRUE}
sub <- ds %>%
  dplyr::select("vote_cand_spt", "ideol", "education", "income", "age", "gender", "f.gender", "glbcc_cert", "f.party", "glbcc_risk", "cncrn_econ") %>%
  na.omit()
```
...
Next, make a plot to explore the relationship between candidate support and ideology; are conservatives more supportive of their candidate than liberals?

```{r 10_plot, echo=TRUE}
ggplot(sub, aes(x = ideol, y = vote_cand_spt)) +
  geom_jitter(alpha = 0.2) +
  geom_smooth(method = "lm", color = "black") +
  geom_smooth(method = "loess")
```

It looks like conservatives are a bit more supportive of their candidate than liberals, but the relationship may be nonlinear. Lets estimate a multiple regression model to check it out:
...
Based on the p-values for the model variables, the ideology variable does not appear to help us understand candidate support; however, we should examine the linearity of the relationships. One way to do this is to plot the residuals by the values of the independent variables. Residual relationships should constitute a straight line with the residuals spread around a line at zero. To build this visualization we:

1. Use `augment()` to predict values and create a data frame containing fitted values and residuals for each observation
...
We can see there are some potential nonlinearities, for instance: the ideology variable. The next step is to consider adding an exponent the ideology variable. __Note:__ We want to avoid over fitting our model to the sample data. Therefore its important to have an understanding of why you are including an exponent.
...
Let's move onto the visualization. Since ideology is our variable of interest, we will visualize the relationship between ideology and candidate support while holding the other variables constant at their means. To do this we will:
...
pol.df <- pol %>%
  augment(newdata = data.frame(ideol = seq(1, 7, 0.1),
...
The next step is to visualize. Use `geom_line()` to create the line, and `geom_ribbon()` to create the confidence interval. For practice, let's label the axes, add a title, and use the `theme_bw`.

```{r 10_poly9, echo=TRUE}
ggplot(pol.df, aes(x = ideol, y = .fitted)) +
...
Let's use an example where a cubic term might be appropriate. A very common model we've used has been exploring the relationship between ideology and climate change risk. Perhaps the relationship between the two is not best described as a linear function. We can explore this with a simple scatter plot:

```{r 10_cube1, echo=TRUE}
ggplot(sub, aes(x = ideol, y = glbcc_risk)) +
  geom_jitter(alpha = 0.2) +
  geom_smooth(method = "lm", color = "black") +
  geom_smooth(method = "loess")
```

Now, let's estimate a model we've looked at many times before and plot the residuals by IV, as we did above.
...
lm1 %>%
  augment() %>%
  melt(measure.vars = c("ideol", "education", "age", "gender")) %>%
...
Looking at the ideology variable, we can see that it is not exactly linear. It looks more like the loess line moves above and below the line at zero. This may imply that a cubic term is appropriate, so lets estimate a model with a cubic term for ideology:
...
It looks like the cube term does not describe the data very well. The adjusted R squared appears to marginally increase in the more complex model, but that does not tell us much. Let's use anova function to conduct a nested F-test:

```{r 10_cube5, echo=TRUE}
anova(lm1, cubed)
```

The F-test tells us that the more complex model is a better fit. This is likely due to the square term, which is statistically significant. Nonetheless, let's visualize this so you have experience seeing cubic functions. 
