...
To start with proportions, We will use the `glbcc` variable from the class data set that contains respondents' opinions on global climate change. Specifically, it asks if they believe that greenhouse gases cause global temperatures to increase. We start with describing the data. A zero indicates *no* and 1 indicates *yes*.
...
Alternatively, we can use the `describe()` function to get an approximate standard errror:
...
```{r 5_prop10, echo = TRUE, fig.height = 3, fig.width = 4, fig.align = "center"}
...
Now that we have the standard errors for men and women, we can return to the previous data frame we constructed and add a column for the standard error. This is done using the tidyverse verb `add_column()`. Assign the new data frame to a new object, then print the data frame:
...
```{r 5_2prop8, echo = TRUE, fig.height = 3, fig.width = 4, fig.align = "center"}
...
## Part III: Independent two-sample t-tests

This section elaborates on two-sample t-tests. There are many different variations of the two-sample t-test that fall into one of two categories: 

**1. Independent two-sample t-test**
**2. Dependent t-test for paired samples**

We use a two-sample independent t-test when the groups that we are comparing are statistically independent (come from seperate samples); we most often use a dependent t-test for paired samples when we have only only one sample that has been tested twice (repeated measures). For now, we are going to concentrate on independent two-sample t-tests.

Technically, the formula we use to calculate an independent two-sample t-test depends on the vairance in each group. If the variance is the same in both groups, we can pool the standard deviation when calculating the t-score and p-value of the test. If the variance is not the same, we use *Welch's t-test* to seperately estimate the variance within each group. By default, the `t.test` function in R assumes unequal variances. We can modify this by changing the `var.equal = FALSE` option to `var.equal = TRUE`.

Lets explore the difference in these methods by comparing Republican and Democratic perceptions of climate change risk. Before we begin, lets check to see of the two groups have roughly equal variances. We do this using the `summarise()` function.

```{r 5_ind2, echo = TRUE, include = FALSE}
...
The sample standard deviation in both groups is different, but we don't know if this difference is statistically different than zero. We can use the Levene test to examine this hypthesis:
...
Given the p-value < $\alpha$ = 0.05, we can reject the null hypothesis and say that there is a statistically significant difference in the two variances. With this result we can perform an independent t-test for two populations. The framework for forming hypotheses is as follows:
...
Though incorrect, here is what we get from the same test if we assume equal variance.

```{r 5_ind6, echo = TRUE}
t.test(ds$glbcc_risk ~ ds$f.part, var.equal = TRUE, alt = "greater")
```

Note that the two tests are very similar, with the only difference being the formula we use to calculate the degrees of freedom. This is important if you are working with relatively small samples.
...
Like traditional independent sample tests involving two samples, we generally assume that the variances within the groups must are unequal, which we can confirm using the Levene test.

```{r 5_hsd, echo = TRUE}
leveneTest(ds$glbcc_risk ~ ds$f.party.2)
```

Given that the p-value < $\alpha$ = 0.05, we reject the null hypothesis that there is *no* difference in variances. The `pairwise.t.test()` function can perform the pairwise test, using the `f.party_2` variable that was factored for Democrats, Republicans, and independents:
...
Given the p-value > $\alpha$ = 0.05, the null hypothesis is not rejected, affirming that the variance is the same within each sample.  
...
```{r 5_vis5, echo = TRUE, fig.height = 3, fig.width = 4, fig.align = "center"}
ggplot(examdf, aes(x = test, y = scores)) +
  geom_point(size = 3, shape = 19, col = "dodgerblue3") + 
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.1, col = "dodgerblue3") + 
  ylim(10, 100) + 
  ggtitle("Exam Scores") + 
  xlab("Exams") + 
  ylab("Scores")
...
```{r 5_vis8, echo = TRUE, fig.height = 3, fig.width = 6, fig.align = "center"}
