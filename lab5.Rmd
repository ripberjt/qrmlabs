...
Similarly to earlier in the lab, confidence intervals can also be calculated by finding the standard error. In this case, we need to find the standard error for men and women separately. We will use the _filter()_ function to simplify the calculations:

```{r 5_2prop3, echo=TRUE}
male <- filter(ds, gender==1)
female <- filter(ds, gender==0)
m.se <- describe(male$glbcc)$se
f.se <- describe(male$glbcc)$se
```
...
Given that the p-value < $\alpha$ = 0.05, we reject the null hypothesis such that there is a difference in variances. The `pairwise.t.test()` function can perform the pairwise test, using the _f.party_2_ variable that was factored for Democrats, Republicans, and independents:
