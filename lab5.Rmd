...
To start with proportions, We will use the _glbcc_ variable from the class dataset that contains respondents' opinions on global climate change. Specifically, it asks if they believe that greenhouse gases cause global temperatures to increase. We start with describing the data. A zero indicates "no" and 1 indicates "yes."
...
Now we can describe the population proportions. To do this we use the _prop.table()_ function. 

```{r 5_prop2, echo=TRUE}
table(ds$glbcc) %>% prop.table()
```
...
```{r 5_prop9, echo=TRUE}
df <- data.frame(answer = c("0", "1"), glbcc = c(no, yes), 
                 lower = c(no.low, yes.low),
...
t.gcc.gend
t.gcc.gend.p <- prop.table(t.gcc.gend)
```
...
To visually examine which gender has, on average, higher belief that greenshouse gases cause climate change, plotting the two mean levels of belief for the genders is a good place to start. To create a visualization, the mean level of _glbcc_ needs to be found for each gender. We will use the _filter()_ function to simplify the calculations:
...
Assign the mean levels of support to objects titled _men2_ and _women2_.

```{r 5_2prop4, echo=TRUE}
men2 <- mean(male$glbcc)
women2 <- mean(female$glbcc)
```

Now the _describe()_ function is used for the standard errors per gender:

```{r 5_2prop5, echo=TRUE}
...
```{r 5_2prop7, echo=TRUE}
df2 <- data.frame(gender = c("Men", "Women"), glbcc = c(men2,women2), 
                 lower = c(men.low,women.low),
...
For this section, we will use the _glbcc\_risk_ variable, that measures the level of risk respondents associate with climate change (on a scale of zero to ten). The range associated to this scale is simplified to zero to five using the _recode()_ function:
...
The _prop.table()_ function describes the relationship by proportions. We convert the proportion to percentage of each response level by gender by including _margin=2_.
...
This requires a different type of data frame. So far the data has been "wide" data, but we need "long" data. The _melt()_ function found in the _dplyr_ package converts data from wide to long format. Include "Exam1" and "Exam2" as the measured variables. 
...
The grouped bar plot is constructed similar to a bar plot, except that the visualization is based on the exam and uses the _position.dodge()_ function to place bars near each other and the _fill=Exam_ argument to specify what the groups 
