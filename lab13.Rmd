...
Use OLS regression to look at the relationship between square footage of a home (DV) and the IVs income, age, and education. First we use _zelig()_ to specify the model, indicate _model="ls"_. For the income variable use logged income. As we've shown many times throughout the labs, the income variable has a skew. 
...
Logged income and age are held at their means, education is sequenced from 1 to 8, and there are three other groups of values. *Qi_ci_min* is the lower limit, *qi_ci_median* is the estimate, and *qi_ci_max* is the upper limit. Let's plot them. Make sure to use _factor(education)_ to treat each level separately. 
...
ds.sub <- filter(ds) %>% select("footage", "trump", "gender",
...
```{r 13_ord6, echo=TRUE}
pv.0 <- setx(logit1, glbcc_risk = 0) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.1 <- setx(logit1, glbcc_risk = 1) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.2 <- setx(logit1, glbcc_risk = 2) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.3 <- setx(logit1, glbcc_risk = 3) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.4 <- setx(logit1, glbcc_risk = 4) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.5 <- setx(logit1, glbcc_risk = 5) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.6 <- setx(logit1, glbcc_risk = 6) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.7 <- setx(logit1, glbcc_risk = 7) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.8 <- setx(logit1, glbcc_risk = 8) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.9 <- setx(logit1, glbcc_risk = 9) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")

pv.10 <- setx(logit1, glbcc_risk = 10) %>% sim() %>% get_qi(qi = "pv", xvalue = "x")
```
...
There are some models that can be specified outside of Zelig but that you can use the Zelig functions on. The complete list can be found on the Zelig website, but we can demonstrate with the _lm()_ function. Here's a classic model from our labs:
