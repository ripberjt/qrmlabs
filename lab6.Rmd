...
```{r 6_cor5, echo=TRUE}
ds.sub <- ds %>%
  select("glbcc_risk", "ideol", "f.gender", "f.party.2") %>%
  na.omit()
```
...
ds %>% 
  select(glbcc_risk, ideol, income, age) %>%
  drop_na() %>%
  cor()
```
...
corr.test(group, use = "complete.obs") %>%
  print(short = FALSE)
```
...
ds %>%
  ...
  select(ideol, glbcc_risk, f.party.2) %>%
  na.omit() %>%
