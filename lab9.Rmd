...
```{r dum, echo=TRUE}
ds.sub <- filter(ds) %>% dplyr::select("ideol", "education", "income",
                                       "age", "gender", "f.gender",
...
1. Generate fitted values and standard errors via the _predict()_ function.
2. Calculate the upper and lower bounds of the confidence interval.
3. Put values into a data frame.

```{r 9_dum4.5, echo=TRUE}
p.w <- predict(lm1, newdata = data.frame(f.gender = "Women",
                                          ideol = seq(1,7,1),
                                          education = mean(ds.sub$education),
                                          income = mean(ds.sub$income),
                                          age = mean(ds.sub$age)), se.fit = TRUE)
p.m <- predict(lm1, newdata = data.frame(f.gender = "Men",
                                          ideol = seq(1,7,1),
                                          education = mean(ds.sub$education),
                                          income = mean(ds.sub$income),
                                          age = mean(ds.sub$age)), se.fit = TRUE)
...
w.low <- w.cert - 1.96 * p.w$se.fit
w.up <- w.cert + 1.96 * p.w$se.fit 
m.low <- m.cert - 1.96 * p.m$se.fit
m.up <- m.cert + 1.96 * p.m$se.fit
cert <- c(m.cert, w.cert)
upper <- c(m.up, w.up)
lower <- c(m.low, w.low)
```

Next you need to put all this information into a data frame. You also need a vector that lists "Men" and "Women" 7 times each:

```{r 9_dem4.8, echo=TRUE}
gend <- rep(c("Men", "Women"), each = 7)
cert.df <- data.frame(cert, upper, lower, ideol = 1:7, gend)
cert.df
```

To separate men and women, you can use _group=DummyVariable_, to separate the two groups. Unfortunately, this does not give a way to distinguish between them. You can also use _color=DummyVariable_ to separate the groups, assign colors, and include a legend. 

```{r 9_dum4, echo=TRUE}
ggplot(cert.df, aes(ideol, cert, color = gend)) +
  geom_line(size = 1) +
  geom_ribbon(ymin = lower, ymax = upper, alpha = .5)
```
...
```{r 9_dum7, echo=TRUE}
values <- predict(lm2, newdata =  data.frame((f.party=c("Dem", "Ind", "Rep", "Other")), 
                                             education=mean(ds.sub$education),
                                             income = mean(ds.sub$ideol), age=mean(ds.sub$age),
...
ggplot(df, aes(x = party, y = cert)) +
  geom_point() +
  geom_errorbar(ymin = lower, ymax = upper, width = 0.1) +
  ylim(4, 8) +
...
```{r 9_dum15, echo=TRUE}
men  <- predict(lm3, newdata =  data.frame((f.party = c("Dem", "Ind", "Other", "Rep")), 
                                           education = mean(ds.sub$education),
                                           income = mean(ds.sub$ideol), age = mean(ds.sub$age),
                                           f.gender = "Men"), se.fit = T)
women  <- predict(lm3, newdata =  data.frame((f.party = c("Dem", "Ind", "Other", "Rep")),
                                             education = mean(ds.sub$education), 
                                             income = mean(ds.sub$ideol), age = mean(ds.sub$age),
...
```{r 9_int3, echo=TRUE}
p.int.m <- predict(lm4, newdata = data.frame(ideol = seq(1,7,1),
                                             f.gender = "Men",
                                             education = mean(ds.sub$education),
                                             income = mean(ds.sub$income),
                                             age = mean(ds.sub$age)), se.fit = T)


p.int.w <- predict(lm4, newdata = data.frame(ideol = seq(1,7,1),
                                             f.gender = "Women",
                                             education = mean(ds.sub$education),
                                             income = mean(ds.sub$income),
                                             age = mean(ds.sub$age)), se.fit = T)

int.w.cert <- p.int.w$fit
int.m.cert <- p.int.m$fit

int.w.low <- int.w.cert - 1.96 * p.int.w$se.fit
int.w.up <- int.w.cert + 1.96 * p.int.w$se.fit 
int.m.low <- int.m.cert - 1.96 * p.int.m$se.fit
int.m.up <- int.m.cert + 1.96 * p.int.m$se.fit

low <- c(int.m.low, int.w.low)
up <- c(int.m.up, int.w.up)
int.cert <- c(int.m.cert, int.w.cert)
gen <- rep(c("Men", "Women"), each = 7)

int.df <- data.frame(ideol = 1:7,
                     gen, int.cert, low, up)
```
...
ggplot(int.df, aes(ideol, int.cert, color = gen)) +
  geom_line(size = 1) +
  geom_ribbon(ymin = low, ymax = up, alpha = 0.5)
```
...
```{r 9_int4, echo=TRUE}
p.int.m$fit[1] - p.int.m$fit[7]
p.int.w$fit[1] - p.int.w$fit[7]
```


We can tell that the first difference is larger for men. They have a higher first value and a higher last value.
...
```{r 9_int8, echo=TRUE}
lm5 <- lm(glbwrm_risk_fed_mgmt ~ ideol * glbcc_risk + education + gender + income 
          + age, data = ds.sub)
summary(lm5)
```
...
```{r 9_int9, echo=TRUE}
lib <- predict(lm5,newdata = data.frame(ideol = (1), gender = mean(ds.sub$gender), 
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age),
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)
...
```{r 9_int10, echo=TRUE}
con <- predict(lm5,newdata = data.frame(ideol = (7), gender = mean(ds.sub$gender),
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age), 
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)
```
...
```{r 9_int15, echo=TRUE}
id1 <-predict(lm5,newdata = data.frame(ideol = (1), gender = mean(ds.sub$gender),
                                     education = mean(ds.sub$education),
                                     income = mean(ds.sub$income), age = mean(ds.sub$age),
                                     glbcc_risk = seq(1, 10, 1)), se.fit = T)

id2 <- predict(lm5,newdata = data.frame(ideol = (2), gender = mean(ds.sub$gender),
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age),
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)

id3 <- predict(lm5,newdata = data.frame(ideol = (3), gender = mean(ds.sub$gender),
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age),
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)

id4 <- predict(lm5,newdata = data.frame(ideol = (4), gender = mean(ds.sub$gender),
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age),
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)

id5 <- predict(lm5,newdata = data.frame(ideol = (5), gender = mean(ds.sub$gender),
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age),
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)

id6 <- predict(lm5,newdata = data.frame(ideol = (6), gender = mean(ds.sub$gender),
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age), 
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)

id7 <- predict(lm5,newdata = data.frame(ideol = (7), gender = mean(ds.sub$gender),
                                      education = mean(ds.sub$education),
                                      income = mean(ds.sub$income), age = mean(ds.sub$age),
                                      glbcc_risk = seq(1, 10, 1)), se.fit = T)
```

Now put all the predicted values into a data frame, along with a sequencing of 1 to 10 for climate change risk, then melt the data into long-form:
...
fit6 <- id6$fit
fit7 <- id7$fit
id.df <- data.frame(fit1, fit2, fit3, fit4, fit5, fit6, fit7, glbcc_risk = 1:10) %>% 
  melt(measure.vars = c("fit1", "fit2", "fit3", "fit4", "fit5", "fit6", "fit7"))
```
...
Now build the visualization. Let's make this a complete visualization, including axis labels, and a title.

```{r 9_int18, echo=TRUE}
ggplot(id.df, aes(glbcc_risk, value, color = variable)) +
  geom_line(size = 2) +
