...
### The Pipe Operator: A Better Way to Work with Data

Sometimes sorting through your data, subsetting it filtering it, etc., can result in a lot of confucing syntax. To avoid this, many people have started using the pipe operator: %>%, found in the _dplyr_ package. In essence, the pipe operator "pipes" one function to the next, chaining it. For example, if you wanted to filter the data to include only the variables "gender", "income", and "age", and then remove any missing obervations, you could do this:

```{r 2_pipe, echo=TRUE}
ds.sub <- filter(ds) %>% select("gender", "income", "age") %>% na.omit()
```

You can read this as a "first this, then this" type of sequence. This tells R "first filter the data, then select the variables, then remove missing observations." Try another example of the pipe function. This time filter the data to include only respondents older than 65, then slice the data to include only the first 500 observations:

```{r 2_pipe2, echo=TRUE}
ds.old <- filter(ds, age > 65) %>% slice(1:500) %>% na.omit()
```
...
In R, the _modeest_ package provides the _mlv()_ function to return the mode for data ta set. __Note:__ The mlv() package does not work for factored variables. To accommodate this limitation, the factored variable can be coerced to a number using the _as.integer()_ function. You also must remove missing observations.

```{r 2_mode1,echo=TRUE}
as.integer(ds$f.party.2) %>% na.omit() %>% mlv()
```
...
```{r 2_mode3, echo=TRUE}
table(ds$f.party.2) %>% sort(decreasing = TRUE)
```
...
The _freq()_ function returns the count in each group and proportions. To construct a table that shows the proportion breakdown only, make a table of the party breakdown, then pipe it to a proportion table, then multiple by 100:
...
To make a visualization with GGplot, first call it with the _ggplot()_ function, then specify what type of visualization you want to make. The steps are:
...
To create a visualization that only includes Democrats, Republicans, and Independents, use the _filter()_ function to filter out NAs, then pipe it to GGplot. Color can also be included by specifying _fill=f.party.2_. 

```{r 2_bar1.5, echo=TRUE}
ds %>% filter(f.party.2 != "NA") %>%
...
Construct one more visualization. This time, include a title by using _ggtitle()_ and axis labels by using _xlab()_ and _ylab()_.
...
```{r 2_ord3, echo=TRUE}
table(ds$f.education) %>% sort(descreasing=TRUE)
```
...
More detail is available by adding colors. There are a few different methods of adding colors using ggplot. Include _fill=f.education_ to indicate that the bars will be filled with color, and *scale_fill_manual()* to indiciate the colors:
...
A histogram is the appropriate visualization for interval data. Histograms returns values on a continuous scale as opposed to individual values. To make a histogram, follow the same basic steps as before, but this time use *geom_histogram()* instead. Also specify *bins=10* and scake the x axis to label each point of the variable:
...
To adjust the bandwidth, use _adjust=_  to specify a value.
...
Last, a box plot can be generated via the _boxplot()_ function. Technically a box plot in GGplot requires both an x and y value, so simply indiciate "" for the x portion of the aesthetic. 

```{r 2_box, echo=TRUE}
ggplot(ds, aes(x = "", y = glbcc_risk)) +
  geom_boxplot()
```
