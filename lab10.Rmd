...
```{r 10_poly, echo=TRUE}
sub <- filter(ds) %>% dplyr::select("vote_cand_spt", "ideol","education",
...
```{r 10_lin3, echo=TRUE}
sub.melt <- melt(sub, measure.vars = c("ideol", "education", "income", "age",
                                       "gender", "mod.fit"))
head(sub.melt)
```
...
p.pol <- predict(poly, newdata = data.frame(ideol = seq(1, 7, .1),
                                            education = mean(sub$education), income = mean(sub$income),
...
up.pol <- p.pol$fit + 1.96 * p.pol$se.fit
low.pol <- p.pol$fit - 1.96 * p.pol$se.fit
```

Add the vectors to a data frame:

```{r 10_poly8, echo=TRUE}
df.pol <- data.frame(ideol=seq(1, 7, .1), p.pol, up.pol, low.pol)
```
...
ggplot(df.pol, aes(ideol, fit)) +
  geom_line(data = df.pol, aes(ideol, fit), size = 1, color = "dodgerblue") +
  geom_ribbon(aes(ymin = low.pol, ymax = up.pol), alpha = .2) +
...
p.cubed <- predict(cubed, newdata = data.frame(ideol = seq(1, 7, .1),
                                             education = mean(sub$education), income = mean(sub$income),
                                             age = mean(sub$age),
                                             gender = mean(sub$gender)), se.fit = T)
...
```{r 10_non3, echo=TRUE}
new.ds <- filter(ds) %>% dplyr::select("income", "gender", "ideol",
                                       "v.trump", "education") %>%
  na.omit()
```
...
```{r 10_mc, echo=TRUE}
data.frame(new.ds$ideol, new.ds$log.inc, new.ds$education) %>% cor()
```
...
```{r 10_ex, echo=TRUE}
d <- filter(ds) %>% dplyr::select("footage", "income", "age", "education") %>% 
  na.omit()
...
```{r 10_ex5, echo=TRUE}
data.frame(d$age,d$education,d$log.inc) %>% cor()
```
