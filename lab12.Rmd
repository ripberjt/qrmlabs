...
logit3 <- glm(f.trump ~ ideol + gender + education + income + race + f.part,
...
Now we combine everything into one data frame using `full_join()`. Recall that `full_join()` only joins two data frames at a time, so you first need to combine two data frames, then pipe that frame into another `full_join()` function:

```{r 12_log32, echo=TRUE, warning=FALSE}
full_join(dem.prob, ind.prob) %>%
  full_join(rep.prob) -> party.df
party.df
```
...
ggplot(party.df, aes(ideol, .fitted, color = as.factor(f.part))) +
  geom_point(size = 1.5) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = .2) +
