...
```{r 12_log2, echo=TRUE}
ds.sub <- filter(ds) %>% dplyr::select("f.trump", "gender", "ideol", "income", "education", "race") %>%
  na.omit()
```
...
ds.sub2 <- filter(ds) %>% dplyr::select("f.trump", "gender", "ideol", "income",
...
```{r 12_ord, echo=TRUE}
ds.sub3 <- filter(ds) %>% dplyr::select("s.energy", "ideol", "age", "glbcc_risk") %>%
  na.omit()
```
